...
function _organizeSwaggerProperties(swaggerObject, pathObject, propertyName) {
	...
	if (propertyName === 'components' || propertyName === 'component') {
		if(!swaggerObject['components']['schemas']) swaggerObject['components']['schemas'] = {};
		let definitionNames = Object
			.getOwnPropertyNames(pathObject['components']['schemas']);
		for (let k = 0; k < definitionNames.length; k = k + 1) {
			let definitionName = definitionNames[k];
			swaggerObject['components']['schemas'][definitionName] = pathObject['components']['schemas'][definitionName];
		}
	}
	// Common properties.
