...
function seekWrong(list, wrongSet, problems) {
	recursive.recursiveIterate(list, (item) => {
		let isDirectChildOfProperties =
			item.path[item.path.length - 2] === "properties";

		if (wrongSet.indexOf(item.key) > 0 && !isDirectChildOfProperties) {
			problems.push(item.key);
		}
	});
}
