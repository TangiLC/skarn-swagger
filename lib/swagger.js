...
function fileFormat(comments) {
    ...
    let route, parameters = {}, params = [], tags = [], components = {}, schemas= {};
    for (let i in comments) {
        ...
        if (i == 'tags') {
            ...
            for (let j in comments[i]) {
                ...
                //if(comments[i][j]['type'])console.log(comments[i][j]['type'].name)
                if (title == 'route') {
    ...
    return {parameters: parameters, tags: tags, components: components, schemas: schemas}
}
